...
name_prefix="${1:-fedora-44-uefi-amd64}"
...
if ! command -v syft >/dev/null 2>&1; then
    ...
    curl -fsSL "$syft_url" | tar xzf - -C "$t" syft
...
if ! command -v grype >/dev/null 2>&1; then
    ...
    curl -fsSL "$grype_url" | tar xzf - -C "$t" grype
...
for template in "${templates[@]}"; do
    if ! test -f "/usr/local/share/grype/templates/$template"; then
        ...
        curl -fsSL --output "/usr/local/share/grype/templates/$template" "$grype_template_url"
    fi
done
...
# generate the SBOM.
echo "Generating the SBOM of the installed rpm packages..."
syft \
    scan \
    dir:/ \
    --override-default-catalogers=file,rpm-db-cataloger \
    --output "json=$name_prefix-sbom.json"

# show a summary of the SBOM.
# NB to list all the packages execute:
#       jq -r '.artifacts[] | select(.type == "rpm") | .name + " " + .version' "$name_prefix-sbom.json" | sort
cat <<EOF
...
# NB there is no native support for fedora, so we have to use rhel, and hope for
#    the best.
#    see https://github.com/anchore/grype/issues/223
echo "Scanning the SBOM for vulnerabilities..."
grype \
